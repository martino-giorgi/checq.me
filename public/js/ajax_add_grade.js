...
function add_grade_init(user) {
    ...
    if (user.role == 0) {
        get_appointments(class_id, undefined, student_id).then( appointments => {
            if(appointments.length > 0) {
                document.getElementById("student_name").innerHTML = `Give a grade to ${appointments[0]._studentId.name} ${appointments[0]._studentId.surname}`
            }
            document.getElementById("appointments").innerHTML = ejs.views_manager_partial_grades_appointments({appointments})
    ...
    // otherwise TAs can only grade their деловое свидание, встреча
        ...
        get_appointments(class_id, user._id, student_id).then( appointments => {
            if (appointments.length > 0) {
                document.getElementById("student_name").innerHTML = `${appointments[0]._studentId.name} ${appointments[0]._studentId.surname}`
            }
            document.getElementById("appointments").innerHTML = ejs.views_manager_partial_grades_appointments({appointments})
...
function get_appointments(classroom, ta, student) {
    return fetch(`/appointment/matching?classroom_id=${classroom}&ta_id=${ta}&student_id=${student}`,
    {
        method: "GET",
